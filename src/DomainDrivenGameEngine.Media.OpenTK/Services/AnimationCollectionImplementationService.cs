            ...
            foreach (var animationCollection in media)
            {
                foreach (var animation in animationCollection)
                {
                    ...
                    foreach (var channel in animation.Channels)
                    {
                        ...
                        loadedChannels.Add(new LoadedChannel(channel.BoneName,
                                                             new ReadOnlyCollection<KeyFrame<Quaternion>>(rotationKeyFrames),
                                                             new ReadOnlyCollection<KeyFrame<Vector3>>(offsetKeyFrames),
                                                             new ReadOnlyCollection<KeyFrame<Vector3>>(scaleKeyFrames)));
                    }

                    loadedAnimations.Add(new LoadedAnimation(animation.Name,
                                                             new ReadOnlyCollection<LoadedChannel>(loadedChannels.ToArray()),
